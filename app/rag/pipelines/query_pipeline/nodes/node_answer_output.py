...
from app.rag.pipelines.query_pipeline.state import QueryGraphState, resolve_trace_key
...
"""
  ...
  入参:  answer(可选) / original_query / rewritten_query / history / item_names / reranked_docs
  出参:  answer / prompt / image_urls
  步骤:
       1. 若 state 已有 answer（确权失败等兜底提示）→ 直接输出，不再调用 LLM
       2. 三区证据（本地 / 图谱 / 联网）+ 历史 组装 Prompt
       3. 调用 LLM 生成答案（支持流式）
       4. 输出侧安全清洗（外链白名单）+ 图片候选交集
       5. 写入 Mongo 历史 + 发送 final 事件
"""
...
def step_2_construct_prompt(state: QueryGraphState) -> str:
    ...
    #    否则会因 cross-encoder 对结构化短句打分偏低而在 RRF/断崖两道闸被淘汰。
...
def step_3_generate_response(state: QueryGraphState, prompt: str) -> QueryGraphState:
    ...
    # 追踪/推送 key：优先 task_id，缺失时自动回退 session_id
...
def _extract_images_from_docs(docs):
    ...
    for i, doc in enumerate(docs):
        ...
        if text:
            # 正则 r'!\[.*?\]\((.*?)\)' 含一个捕获组，findall 只返回括号内匹配到的内容（即 URL）
...
def node_answer_output(state: QueryGraphState) -> QueryGraphState:
    ...
    # 回写 state，供上层兜底 FINAL 事件读取，保证流式/非流式一致
...
if __name__ == "__main__":
    ...
    # 注意：来源标识字段是 type（milvus=本地知识库 / kg=本地图谱关系 / web=联网结果）。
    # 重排节点不产出 source / chunk_id，mock 数据需与真实结构一致，
    # 否则会全部落到最低信任的联网区。
    mock_reranked_docs = [
        {
            "type": "milvus",
    ...
    mock_state = create_query_default_state(
        ...
        task_id="test_answer_session_001",
