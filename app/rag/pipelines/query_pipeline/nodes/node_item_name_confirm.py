...
"""
  节点: 商品名确认 (node_item_name_confirm)
  入参:  original_query / session_id  (+ task_id 用于追踪)
...
def step_3_llm_itemnames_and_rewrite(history_chats, original_query):
    ...
    :param history_chats: 历史消息列表
    :param original_query: 用户原始问题
    :return: {item_names:[], rewritten_query:重写问题}
    """
    # 1. 初始化模型对象（json_mode：要求模型返回结构化 JSON）
    llm_client = get_llm_client(json_mode=True)
    # 2. 进行提示词处理
    #    加固点：历史记录的 item_names 可能为 null（历史数据或其它写入路径未回填），
    #    此处统一兜底为空列表，避免 '、'.join(None) 抛 TypeError。
    history_text = ""
    for msg in history_chats:
        related = "、".join(msg.get("item_names") or [])
        role = msg.get("role")
        content = msg.get("rewritten_query") if role == "user" else msg.get("text")
        history_text += f"角色:{role},内容:{content},关联主体: {related}\n"
...
def step_6_deal_state(state, final_result, rewritten_query, history_chats):
    """
    修改state : answer  item_names rewritten_query history
    :param state:
    :param final_result:
...
def node_item_name_confirm(state):
    ...
    输出：更新 state['item_names']；未确权时写入 state['answer'] 作为兜底提示
    """
    #  1. 日志和任务处理 is_stream
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("\n>>> 测试完成！最终状态:")
        print(json.dumps({k: v for k, v in result_state.items() if k in ("item_names", "rewritten_query", "answer")},
