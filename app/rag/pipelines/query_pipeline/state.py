...
def create_query_default_state(**overrides) -> QueryGraphState:
    ...
    Returns:
        新的状态实例
    Examples:
        state = create_query_default_state(session_id="s1", original_query="xxx")
    """
...
# ========================
# 追踪 key 解析（task_id / session_id 分离）
# ========================
def resolve_trace_key(state) -> str:
    """
    解析「任务追踪 / SSE 队列」key。

    统一使用 task_id（一个会话内可并发多轮查询互不覆盖）；
