"""
导入链路可调参数配置（对应 import pipeline 各节点）

说明：
    1. 本文件只承载"业务可调参数"，不承载连接信息（连接类配置见 mineru_config /
       minio_config / milvus_config 等）；
    2. 每个字段均可用同名环境变量覆盖；
...
# ==================== 知识图谱白名单（领域知识，与 app/prompts/templates/knowledge_graph.prompt 保持一致）====================
...
def _validate_import_pipeline_config(cfg: ImportPipelineConfig) -> None:
    ...
    # 3. 合并阈值不能大于切分触发阈值，否则「刚切完就被合并」形成无谓处理
