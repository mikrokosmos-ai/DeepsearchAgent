"""
提示词模板加载器 —— 项目内 .prompt 模板的唯一读取入口

模板与代码同包存放：app/prompts/templates/<name>.prompt
（随包分发，不依赖项目根目录探测结果）
"""
import string
from pathlib import Path

from app.core.logger import logger  # 同时负责加载 .env（副作用）

# 模板目录：与本文件同包，搬迁/换工作目录都不受影响
