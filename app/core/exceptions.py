...
为「配置 → 客户端 → 导入链路 → 查询链路」提供一套可辨识的领域异常，
让上层（API 路由 / 后台任务 / 未来的重试与降级逻辑）可以按异常类型做精确处理，
而不必再去匹配 str(e) 的内容。
...
设计说明：
    ...
    2. 按链路分四支，避免"一个大而全的异常类"：
       - 配置 / 客户端支：ConfigurationError、ClientInitError
       - 文档链路支    ：FileProcessingError（含 Pdf/Image 细分）、DocumentSplitError、
                        EmbeddingError、IndexingError、KgImportError
       - 查询链路支    ：SearchError（含 VectorSearchError）、RerankError、
                        ItemNameConfirmError、LLMError、StateFieldError
       - 存储支        ：StorageError（含 Milvus/Minio/Mongo/Neo4j 细分）
    3. 提供 AppError.wrap() 统一包装入口，供 app.core.logger.node_guard 装饰器复用。

注意事项（重要，请勿"顺手优化"）：
    1. **Pydantic 校验器必须继续抛 ValueError**，不要改成这里的领域异常。
       原因：Pydantic v2 只把 ValueError / AssertionError 转换为 ValidationError
       并由 FastAPI 返回 422；换成非 ValueError 子类的自定义异常会直接穿透校验层，
       把 422 变成 500，前端的错误处理逻辑会随之失效。
...
class ConfigurationError(AppError):
    ...
    典型场景：MILVUS_HOST 未配置、OPENAI_API_KEY 为空、数值型配置无法转 int。
    """
    pass
...
class ClientInitError(AppError):
    """
    客户端初始化错误：外部依赖（Milvus / Mongo / MinIO / Neo4j / 本地模型）建连或加载失败
...
class PdfConversionError(FileProcessingError):
    """PDF 转换错误：MinerU 提交/轮询/下载失败，或解析超时"""
    pass
...
class IndexingError(AppError):
    """索引写入错误：Milvus 建集合 / 插入 / 加载失败"""
    pass


class KgImportError(AppError):
    """知识图谱导入错误：实体抽取、Neo4j 写入失败"""
    pass
