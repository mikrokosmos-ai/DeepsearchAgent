...
def generate_markdown(
    ...
    logger.debug(f"[MarkdownTool] 当前会话目录：{session_dir}")
    ...
    logger.debug(
        f"[MarkdownTool] parent_dir={parent_dir}, filename={filename}, full_path={file_path}"
    )
    ...
    try:
        ...
        if not parent_dir.exists():
            ...
            logger.debug(f"[MarkdownTool] 已创建目录：{parent_dir}")
        ...
        logger.error(f"[MarkdownTool] 文件写入失败：{e}")
